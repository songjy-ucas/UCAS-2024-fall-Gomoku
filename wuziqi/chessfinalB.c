...
void bubbleSort();//排序
void bubblesort1();//排序2.0，更快；
void InitBoardArray();  //��ʼ��������
void DisplayBoard();//展示棋盘
int iswin();//判断胜利函数
void playGame1();//人人
void playGame2();//人机
int isValidMove();//判断是否越界
void bannewLONG();//长连禁手
void bannewsisi();//四四禁手
void bannewsansan();//三三禁手
int playerchange();//黑白棋转换函数
void evaluate();//赋分
int  startchoose();//遍历全棋盘，将分差，第一步第二步信息记录在maxmax中，返回maxmax序列长度
		...
		if(AIplayer == currentplayer){
			...
			for (int i = 0; i < MAXSIZE; i++) {
				...
				if(round==1 ){
					...
					huiqi[round-1].x = x;
				    huiqi[round-1].y = z;
				    huiqi[round-1].score = Board[x][z];//天元下棋，还有将数存进悔棋数组以方便悔棋
				}else{
			...
			huiqi[round-1].x = x;
			huiqi[round-1].y = z;
			huiqi[round-1].score = Board[x][z];//写进悔棋函数里
		}
		if(currentplayer == BLACK){
		 for(int i=0;i<=SIZE;i++){
		    for(int j=0;j<=SIZE;j++){
...
void evaluate(int Board[SIZE][SIZE],int player,int score1[SIZE][SIZE],int score2[SIZE][SIZE] ){//赋分函数
    int directions[4][2] = {{1,0}, {0,1}, {1,1}, {1, -1}};
    ...
    for(int i =0;i<15;i++){
        for(int j=0;j<15;j++){
            ...
            if(Board[i][j]/10 == 0  ||(Board[i][j]== BAN && player == WHITE)){
                ...
                Board[i][j] = player;//如果是可以下的位置直接下
            }else {
                t[i][j] = -infinite;//不可以下直接赋最低
				if(player == WHITE){
					score2[i][j] = t[i][j];
            ...
            if(iswin(Board, player, i, j)==1){
				...
				Board[i][j] = m;
                continue;//跳过后面的判断
            }else {//单子相加，不考虑棋型，本来想的是会是一种更为宏观的下法，会更好，但是最后发现赋分函数难，且有些复杂
                int sumt =0;//总分
				int lian3 = 0;//连棋为三个的
				int huolian3 = 0 ;//活三
				int huolian4 = 0 ;//活四
                ...
                for(int k=0;k<4;k++){
                    ...
                    int countlian=1;//与所下棋是否连在一起
					int escape1 = 0;//是否有对方棋子阻挡
					int escape2 = 0;//第二个方向阻挡
					int countok = 1;//判断这一行是否有五个以上的空格
                    int specialescape1 = 0;//是否是被紧贴着阻挡
                    int specialescape2 = 0;//同理，第二行。主要是为了在1 3 棋型能够断住别人
                    int countscore = 100;//分数
                    int flag = 1;//判断棋子是否连起来的符号
                    int count = 0;//不与所下子连起来的连棋
					int count3 = 0;//跳着的即1 3类
                    ...
                    for(int l=1;l<6;l++){                      
                        ...
                        if(nx >0 && nx <SIZE &&ny >0 && ny <SIZE){
                            if(Board[nx][ny]/5 == player/5){//除以五的操作是为了保证white与whitetem是统一的以及black与blacktem，而不受禁手BAN影响
                                if(flag == 1){
                                    countlian++;
									countok++;
				...
				}//取位置更好的，但是如果连起来三个的就没必要了
				if(countlian >1){
					specialescape1 = 0; //我们要单子断别人，所以第一个方向的countlian不一定是最终的countlian，需要再检验
				}
				if(countok >4){
                    ...
                    //主要考虑单子断住别人加强防守（虽然决策函数其实也起到防守之用）
                    if((specialescape1 == 1)&&(specialescape2 == 0)){
                        int count1 = 0;
						int countok = 0;
                        int countspecial = 0;
                        int sx = i - 1*directions[k][0];
                        int sy = j - 1*directions[k][1];
                        if(sx >0 && sx <SIZE &&sy >0 && sy <SIZE){
                        if((Board[sx][sy]/10 == 0)||(player == WHITE && Board[sx][sy] == BAN)){
								countspecial = 1;
                           }
                        }//判断旁边是否有空位
                       for(int l = 1;l<6;l++){
				...
				if(player==WHITE){
					...
					if(huolian3 == 1&&huolian4 == 1){
						sumt *=2;//考虑黑棋有禁手，更复杂赋分
					}
				}
                if(banhuosi >1){
                    sumt += 62500;//两个半活四就要挡了
                }
                t[i][j] = sumt;
                Board[i][j] = m;//复原
                if(player == WHITE){
        ...
        for(int j =0;j<SIZE;j++){
            ...
            if(currentplayer == BLACK){
                if(score1[i][j]== -infinite){
                    ...
                    maxmax[k]= different;//将信息存储在maxmax里面
                    k++;
                    Board[i][j]=l;
...
struct delt max201[getit], struct delt max202[getother], struct delt max203[MAXSIZE],int round){
	...
	bubblesort1(maxmax,k1,getit);//全新2.0排序，更快一点
    ...
    for(int i = 0;i<getit;i++){
        if(i!=k1){
            ...
            Board[need1.x][need1.y] = currentplayer;//虚拟下一步
            p1 = playerchange(currentplayer);//考虑白棋的最好部署
			...
			bubblesort1(maxmax,u,1);//这个只需要最好的那个就行了，所以值拿一
            if(u == 0){
...
void bubblesort1(struct delt maxmax[MAXSIZE],int u,int get/*get是我们所取的最优棋子数目*/){
	if(u>get){
